...
namespace FuelEase.PKElectronics
{
    public class PKElectronicsFuelDispenser : IFuelDispenserService
    {
            ...
            IEnumerable<Nozzle>? nozzles)
        {
            ...
            _parser = protocolParserFactory.CreateIProtocolParser(ControllerType.PKElectronics, _nozzles);
        }
        ...
        private async Task ReadingDataFromScreen(Nozzle nozzle)
        {
            ...
            if (response.IsValid)
            {
                ...
                decimal liters = combinedValue / 1000.0; // Преобразуем в литры и граммы (3030 -> 3.030 литра)
