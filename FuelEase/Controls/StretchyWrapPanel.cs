...
namespace FuelEase.Controls
{
    public class StretchyWrapPanel : StackPanel
    {
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            for (int i = 0, count = children.Count; i < count; i++)
            {
                ...
                if (curLineSize.U + sz.U > uvFinalSize.U)
                {
                    ...
                    {
                        ArrangeLine(accumulatedV, curLineSize.V, firstInLine, i, true, useItemU ? itemU : uvFinalSize.Width / Math.Max(1, i - firstInLine - 1), finalSize.Height);
                    }
                    ...
                    if (sz.U > uvFinalSize.U)
                    {
                        ...
                        {
                            ArrangeLine(accumulatedV, sz.V, i, ++i, true, useItemU ? itemU : uvFinalSize.Width, finalSize.Height);
                        }
            ...
            if (firstInLine < children.Count)
            {
                ...
                else
                {
                    ArrangeLine(accumulatedV, curLineSize.V, firstInLine, children.Count, true,
                            useItemU ? itemU : uvFinalSize.Width / Math.Max(1, children.Count - firstInLine - 1), finalSize.Height);
                }
            }
        ...
        private void ArrangeLineProportionally(double v, double lineV, int start, int end, double limitU)
        {
            ...
            // Вычисление общего размера всех элементов на строке
            var total = 0d;
            for (var i = start; i < end; i++)
            {
                ...
                if (child != null)
                {
                    if (double.IsNaN(horizontal ? child.DesiredSize.Width : child.DesiredSize.Height))
                    {
                        throw new InvalidOperationException("Invalid DesiredSize for child element.");
                    }

                    total += horizontal ? child.DesiredSize.Width : child.DesiredSize.Height;
                }
            }

            // Защита от деления на ноль
            var uMultiplier = total > 0 ? limitU / total : 0;
            ...
            for (var i = start; i < end; i++)
            {
                ...
                if (child != null)
                {
                    ...
                    var layoutSlotU = childSize.U * uMultiplier;

                    // Проверяем, чтобы размер layoutSlotU был допустимым
                    if (double.IsNaN(layoutSlotU) || layoutSlotU < 0)
                    {
                        throw new InvalidOperationException("Calculated layout slot size is invalid.");
                    }

                    // Вызываем метод Arrange для текущего элемента
                    child.Arrange(new Rect(
                        horizontal ? u : v,                     // x-координата для горизонтальной ориентации
                        horizontal ? v : u,                     // y-координата для вертикальной ориентации
                        horizontal ? layoutSlotU : lineV,       // ширина для горизонтальной ориентации
                        horizontal ? lineV : layoutSlotU        // высота для вертикальной ориентации
                    ));

                    // Смещение для следующего элемента
                    u += layoutSlotU;
                }
            }
        }

        private void ArrangeLine(double v, double lineV, int start, int end, bool useItemU, double itemU, double totalHeight)
        {
            var u = 0d;
            var horizontal = Orientation == Orientation.Horizontal;
            var children = InternalChildren;
            ...
            for (var i = start; i < end; i++)
            {
                ...
                if (child != null)
                {
                    ...
                    // Используем totalHeight, чтобы занять всю доступную высоту
                    var layoutSlotV = horizontal ? lineV : totalHeight;

                    child.Arrange(new Rect(
                        horizontal ? u : v, 
                        horizontal ? v : u,
                        horizontal ? layoutSlotU : lineV, 
                        horizontal ? layoutSlotV : layoutSlotU));
                    u += layoutSlotU;// Смещаемся вправо для следующего элемента
                }
            }
        }
    }
